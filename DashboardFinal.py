...
def fan_Image(tempValue, tempThreshold):
...
def powerBtnStatus(tempValue, tempThreshold):
    if tempValue >= tempThreshold:
        buttonStatus = 'True'
        return buttonStatus
    else:
...
def powerBtnColor(tempValue, tempThreshold):
    if tempValue >= tempThreshold:
        buttonColor = 'green'
        return buttonColor
    else:
...
powerButton = html.Div([
    # 0 is the default value
                ...
                label='Fan',
                disabled='True',
                on=powerBtnStatus(0,0),
                color=powerBtnColor(0,0),
...
app.layout = html.Div(style={'background-color': '#1D1D1E', 'height':'1000px', 'width':'100%', 'position':'fixed'}, children=[
    daq.DarkThemeProvider(theme=theme, children=cardDisplay), topBar, toast
    ])
...
#     print("Message received-> " + msg.topic + " " + str(msg.payload))  # Print a received msg
#     global rfidPayload
#     rfidPayload = msg.payload.decode()
...
# user = getDefaultUser()
# lightThreshold = user['lightThreshold']
...
#               Output('graph', 'xAxis'), Output('graph', 'yAxis'),              
#     Input('interval-component', 'n_intervals'), Input('toast', 'is_open'))
# def update_output(value, value2):
#     print("Running Callback")
#     # Global variables
#     global isLightEmailSent, isOpen, isTemperatureEmailSent, userRespondedYes, user
#     global lightThreshold, temperatureThreshold
#     isOpen = value2
...
#     fan_Image(lightNumber, lightThreshold), powerBtnStatus(temperatureNumber, temperatureThreshold), 
#     powerBtnColor(temperatureNumber, temperatureThreshold), xAxis, yAxis
